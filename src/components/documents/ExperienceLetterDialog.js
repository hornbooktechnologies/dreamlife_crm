...
const LetterGenerationDialog = ({ isOpen, setIsOpen, employee }) => {
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Experience_Letter_${formData.name?.replace(/\s+/g, "_")}`,
  });
      ...
      <DialogContent className="max-w-4xl w-[95vw] h-[90vh] overflow-hidden p-0 bg-white border-0 shadow-2xl flex flex-col text-left [&>button]:!text-white [&>button]:top-5 [&>button]:right-5 [&>button]:focus:ring-0 [&>button]:focus:outline-none [&>button]:transition-all [&>button>svg]:w-6 [&>button>svg]:h-6 [&>button>svg]:stroke-[3]">
        <DialogHeader className="p-6 bg-gradient-to-r from-primary via-primary-hover to-primary text-white shrink-0 !text-left">
          ...
          <p className="text-blue-50 text-sm mt-1 text-left">
            Review and edit details before generating the letter for{" "}
            <b>{formData.name}</b>.
          </p>
        </DialogHeader>
              ...
              <div className="grid gap-2">
                ...
                <div className="relative group">
                  ...
                  <Input
                    ...
                    value={formData.relievingDate ? format(formData.relievingDate, "dd-MM-yyyy") : ""}
                  ...
                  <Input
                    type="date"
                    id="relievingDate"
                    value={formData.relievingDate ? format(formData.relievingDate, "yyyy-MM-dd") : ""}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        relievingDate: e.target.value ? new Date(e.target.value) : null,
                      })
                    }
            ...
            <div className="border shadow-sm rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center p-4">
              <div className="relative w-full h-[300px] flex justify-center">
                <div className="absolute top-0 scale-50 origin-top transform-gpu h-[600px] w-[800px] overflow-hidden bg-white shadow-2xl">
                  {/* We use a hidden div for the actual print, and a scaled version for preview to avoid layout issues */}
                  <div className="pointer-events-none select-none">
                    <ExperienceLetterTemplate
                      data={{
                        ...formData,
                        relievingDate: formData.relievingDate
                          ? format(formData.relievingDate, "PPP")
                          : "",
                      }}
                    // No ref here for preview to prevent double ref assignment issues if not needed,
                    // or we can just render it. The actual print uses the hidden one below.
                    />
                  </div>
                </div>
              </div>
